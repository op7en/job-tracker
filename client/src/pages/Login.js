...
const Login = () => {
    ...
    const handleSubmit = async () => {
        try {
            const res = await axios_1.default.post("/auth/login", { email, password });
    ...
    return (<div className="min-h-screen bg-gray-900 flex items-center justify-center">
      <div className="bg-gray-800 p-8 rounded-lg w-full max-w-md">
        ...
        <button onClick={() => navigate("/register")} className="w-full text-gray-400 hover:text-white text-sm">
          No account? Register
        </button>
      </div>
    </div>);
